...
export default function RAJUIApp() {
  ...
  const updatePreview = (codeString: string) => {
    if (!iframeRef.current) {
      console.warn("[v0] Cannot update preview: iframe ref not available")
      return
    }

    console.log("[v0] Updating preview, code length:", codeString.length)
    ...
    console.log("[v0] Cleaned code length:", cleanedCode.length)
...
<body>
    ...
    <script type="text/babel">
        // Expose React and ReactDOM APIs
        const { createElement } = React;
        const { createRoot } = ReactDOM;

        // Expose React hooks
        const { useState, useEffect, useCallback, useMemo, useRef, useContext, useReducer, useLayoutEffect } = React;

        // Polyfill for clsx - simple className merger
        const clsx = (...args) => {
            return args
                .flat()
        ...
        // Polyfill for classnames (alias for clsx)
        const classnames = clsx;
        const classNames = clsx;

        try {
            // Execute the cleaned generated code (imports/exports removed)
            ${cleanedCode}

            // Find the component - handle different export patterns
            ...
            if (!ComponentToRender) {
                throw new Error('No component found. Make sure to export a component named "Component" or use "export default".');
            }

            // Create a wrapper to handle props and provide sample data
            const App = () => {
                ...
                const sampleProps = {
                    ...
                    description: 'This is a sample description text that demonstrates how the component looks with content.',
                ...
                return createElement(ComponentToRender, sampleProps);
            };
